//MUST HAVE - CREATE A TEMPLATE TAG
var template_bg = document.createElement("template"); //<template> </template> RULE

//To-do - CREATE THE UI HERE!
template_bg.innerHTML = `
<style id='mystyle'>
  #bg {
    position:fixed;
    left:0;
    top:0;
    z-index:-1;
    width:100vw;
    height:100vh;
  }

  #bg img {
    width:100%;
    height:100%;
    object-fit:cover;
  }

</style>
<div id='bg'>
  <img src='./imgs/desert.svg' />
</div>
`;

//MUST HAVE - CREATE A CLASS WITH HTMLELEMENT POWERS (interfaces/functionalities)
class GameBG extends HTMLElement {

    //MUST HAVE - CREATE A CONSTRUCTOR TO DO INITAL ASSOCIATIONS
    constructor(){
        super(); //pass on the HTMLElement super powers!
        this.attachShadow({mode:"open"}) //Attach it to the shadowRoot

        //To-do - CREATE THE STATES FOR THE UI HERE!
    }

    //MUST HAVE - FUNCTION THAT RUNS AFTER IT'S CONNECTED
    connectedCallback(){
        this.shadowRoot.appendChild(template_bg.content.cloneNode(true)); //use the template to make a clone
        // this.shadowRoot.querySelector("#bg > img").src = 'imgs/forest.svg'
        // this.shadowRoot.querySelector("#bg").onclick = () => this.changeBG();

        /* if (this.getAttribute("bg")) {
          this.shadowRoot.querySelector("#bg img").src = this.getAttribute("bg");
      } */
    }

    //To-do - CREATE THE FUNCTIONALITIES HERE!
    changeBG(img='./imgs/snow.svg'){
      this.shadowRoot.querySelector("#bg > img").src = img;
      
}
    
}

//MUST HAVE - define the tag for the custom elements
customElements.define("game-bg", GameBG)